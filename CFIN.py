...
class CFIN(nn.Module):
    def __init__(self, channels=1, num_classes=16, drop=0.1):
        ...
        norm = nn.BatchNorm2d
        act = nn.ReLU
        self.msfi = MSFI(input_channels_list, fused_channels, norm, act)

        self.conv1 = nn.Conv2d(128, 64, kernel_size=(1, 1))
        self.conv2 = nn.Conv2d(64, 32, kernel_size=(1, 1))
        self.conv3 = nn.Conv2d(32, 16, kernel_size=(1, 1))
    ...
    def forward(self, x):
        ...
        x = self.haf4(x)
        x_4 = x
        x = self.msfi([x_1, x_2, x_3, x_4, shortcut])
...
if __name__ == '__main__':
    model = CFIN(channels=1, num_classes=9)
